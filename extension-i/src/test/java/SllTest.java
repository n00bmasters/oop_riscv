...
public class SllTest extends TestExtension {

    @ParameterizedTest
    @MethodSource("TestExtension#xlenValues")
    void testLargeShift(int xlen) {
        BigInteger rs1Value = BigInteger.valueOf(100);
        BigInteger rs2Value = BigInteger.valueOf(32);
        setup(xlen);
        int rd = 10;
        int rs1 = 11;
        int rs2 = 12;
        ...
        BigInteger expected = rs1Value.shiftLeft(rs2Value.intValue()).and(mask);
